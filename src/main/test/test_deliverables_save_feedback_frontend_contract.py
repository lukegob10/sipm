...
def test_deliverables_forms_mark_required_fields():
    ...
    assert (
        text.count(
            'Fields marked <span class="required-marker" aria-hidden="true">*</span> are required.'
        )
        == 3
    )
    assert (
        '<span class="field-label">Project<span class="required-marker" aria-hidden="true">*</span></span><input name="project_name" required />'
        in text
    )
    assert (
        '<span class="field-label">Sponsor<span class="required-marker" aria-hidden="true">*</span></span><input name="sponsor" required />'
        in text
    )
    assert (
        '<span class="field-label">Project<span class="required-marker" aria-hidden="true">*</span></span>'
        in text
    )
    assert (
        '<span class="field-label">Solution<span class="required-marker" aria-hidden="true">*</span></span><input name="solution_name" required />'
        in text
    )
    assert (
        '<span class="field-label">Solution Owner<span class="required-marker" aria-hidden="true">*</span></span><input name="owner" required />'
        in text
    )
    assert (
        '<span class="field-label">Task<span class="required-marker" aria-hidden="true">*</span></span><input name="subcomponent_name" required />'
        in text
    )
...
def test_subcomponent_form_footer_is_managed_outside_scroll_flow():
    ...
    assert (
        '#solution-modal .modal-tab[data-tab-panel="subcomponents"].active {'
        in styles_text
    )
    assert (
        '#solution-modal .modal-tab[data-tab-panel="subcomponents"] > .subcomponent-tab-scroll {'
        in styles_text
    )
...
def test_solution_form_uses_inline_feedback_for_required_project_and_failures():
    solution_text = SOLUTION_ENTITIES_JS.read_text(encoding="utf-8")
    solution_section = solution_text.split("function bindSolutionForm() {", 1)[1].split(
        "return {", 1
    )[0]
...
def test_project_form_uses_inline_feedback_for_failures():
    project_text = PROJECT_ENTITIES_JS.read_text(encoding="utf-8")
    project_section = project_text.split("function bindProjectForm() {", 1)[1].split(
        "return {", 1
    )[0]

    assert "setDeliverableFormNotice(" in project_section
    assert (
        '`${isEditing ? "Save" : "Create"} failed: ${err.message}`' in project_section
    )
    assert (
...
def test_subcomponent_form_uses_inline_feedback_for_failures():
    subcomponent_text = SUBCOMPONENT_ENTITIES_JS.read_text(encoding="utf-8")
    subcomponent_section = subcomponent_text.split(
        "function bindSubcomponentForm() {", 1
    )[1].split("return {", 1)[0]
    ...
    assert (
        'alert(`${isEditing ? "Save" : "Create"} failed: ${err.message}`);'
        not in subcomponent_section
    )


def test_solution_phases_use_inline_feedback_for_failures():
    js_text = APP_JS.read_text(encoding="utf-8")
    phases_section = js_text.split(
        "async function renderSolutionPhases(selectedId) {", 1
    )[1].split("function bindSubcomponentForm() {", 1)[0]
