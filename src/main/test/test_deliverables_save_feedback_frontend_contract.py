...
def test_escape_returns_from_task_editor_before_closing_solution_modal():
    app_text = APP_JS.read_text(encoding="utf-8")
    task_text = TASK_ENTITIES_JS.read_text(encoding="utf-8")

    assert "function hideTaskForm() {" in task_text
    assert 'if (els.taskForm && !els.taskForm.classList.contains("hidden")) {' in app_text
    assert "hideTaskForm();" in app_text
