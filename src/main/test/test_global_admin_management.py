...
@pytest.mark.anyio
async def test_global_admin_role_endpoints_by_user_id_and_soeid(
    ...
    grant_by_id = await client.post(
        "/project-manager/api/users/regular-user-1/global-admin"
    )
    assert grant_by_id.status_code == 200, grant_by_id.text
    assert grant_by_id.json()["role"] == "global_admin"

    grant_by_soeid = await client.post(
        "/project-manager/api/users/by-soeid/user1/global-admin"
    )
    assert grant_by_soeid.status_code == 200, grant_by_soeid.text
    assert grant_by_soeid.json()["role"] == "global_admin"

    revoke_by_soeid = await client.delete(
        "/project-manager/api/users/by-soeid/user1/global-admin"
    )
...
@pytest.mark.anyio
async def test_cannot_revoke_last_active_global_admin_via_global_admin_endpoint(
    client, db_sessionmaker
):
    with db_sessionmaker() as session:
        admin = User(
    ...
    blocked = await client.delete(
        "/project-manager/api/users/only-global-admin/global-admin"
    )
...
@pytest.mark.anyio
async def test_non_global_admin_cannot_manage_global_admin_role(
    client, db_sessionmaker
):
    _seed_global_admin_users(db_sessionmaker)
...
@pytest.mark.anyio
async def test_global_admin_routes_accept_legacy_formatted_role_values(
    client, db_sessionmaker
):
    with db_sessionmaker() as session:
        legacy = User(
