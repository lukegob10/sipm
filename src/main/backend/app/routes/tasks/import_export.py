...
from ...models import Project, Solution, Task, User
...
def import_tasks(
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        if not project_name or not solution_name or not task_name:
            ...
            continue
        key = (project_name.lower(), solution_name.lower(), version_raw.lower(), task_name.lower())
        if key in seen:
            errors.append(
                f"Row {idx}: duplicate task '{task_name}' for solution '{solution_name}' in project '{project_name}' (strict-first policy)"
            )
            continue
        seen.add(key)
        ...
        try:
            status_enum = normalize_status(
                row.get("status") or TaskStatus.to_do.value, TaskStatus
            )
        ...
        try:
            ...
            if existing:
                if assignee_input:
                    resolved_assignee, resolved_assignee_user_soeid = _resolve_task_assignee(
                        assignee_input,
                        assignee_user_soeid,
                        current_user,
                    )
                ...
                if status_enum == TaskStatus.complete and completed_at_val is not None:
...
def export_tasks(
    ...
    tasks = _task_query(session, space_ctx).order_by(Task.created_at.asc()).all()
    ...
    writer.writeheader()
    for task in tasks:
        solution_name, solution_version = solution_map.get(task.solution_id, ("", ""))
        writer.writerow(
            {
                ...
                "assignee_user_soeid": task.assignee_user_soeid or "",
                "github_repo_url": task.github_repo_url or "",
