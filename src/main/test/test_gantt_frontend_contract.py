...
GANTT_INTERACTIONS_JS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "gantt" / "interactions.js"
)
...
def test_gantt_route_is_registered_and_rendered():
    ...
    assert (
        'type="button" data-view="gantt" class="nav-btn">Gantt</button>' in index_text
    )
    work_section = index_text[
        index_text.index('<p class="nav-label">Work</p>') : index_text.index(
            '<p class="nav-label">Insight</p>'
        )
    ]
    insight_section = index_text[
        index_text.index('<p class="nav-label">Insight</p>') : index_text.index(
            '<div id="nav-admin-section"'
        )
    ]
...
def test_gantt_contract_uses_existing_dates_rollups_and_drilldowns():
    ...
    assert (
        "dateRangesOverlap(startDay, endDay, windowStartDay, windowEndDay)"
        in gantt_text
    )
    assert "buildGanttHealthContext" in gantt_text
    assert (
        "buildProjectNode(project, childNodes, healthContext, todayDay)" in gantt_text
    )
    assert (
        "buildSolutionNode(solution, childNodes, windowRange, healthContext, todayDay)"
        in gantt_text
    )
    ...
    assert (
        "state.ganttCollapsed.add(`project:${project.project_id}`);"
        in interactions_text
    )
    assert (
        "state.ganttCollapsed.add(`solution:${solution.solution_id}`);"
        in interactions_text
    )
...
def test_gantt_styles_are_route_scoped_and_scrollable():
    ...
    assert (
        "grid-template-columns: var(--gantt-left-width) var(--gantt-track-width);"
        in gantt_css_text
    )
