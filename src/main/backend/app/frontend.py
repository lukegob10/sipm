...
from backend.app.paths import APP_CONTEXT_PATH, RESET_PASSWORD_PATH, app_path, app_root_path
...
def frontend_bundle_error(
    *,
    ...
    missing = [path.relative_to(base_dir).as_posix() for path in required_files if not path.exists()]
    if not missing:
        return None
...
class FrontendServer:
    def __init__(
        ...
        *,
        ...
        self.static = StaticFiles(directory=frontend_dir, html=False) if frontend_dir.exists() else None
    ...
    def bundle_error(self) -> str | None:
        ...
        return frontend_bundle_error(base_dir=self.base_dir, required_files=self.required_files)
