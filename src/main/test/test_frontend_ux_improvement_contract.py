...
PROJECT_ENTITIES_JS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "entities" / "projects.js"
)
SUBCOMPONENT_ENTITIES_JS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "entities" / "subcomponents.js"
)
SOLUTION_ENTITIES_JS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "entities" / "solutions.js"
)
CALENDAR_INTERACTIONS_JS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "calendar" / "interactions.js"
)
KANBAN_INTERACTIONS_JS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "kanban" / "interactions.js"
)
TEAM_CAPACITY_INTERACTIONS_JS = (
    REPO_ROOT
    / "src"
    / "main"
    / "ui"
...
MASTER_ROUTE_QUICKSTART = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "master" / "quickstart.js"
)
...
PLANNING_COMMON = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "planning" / "common.js"
)
PLANNING_STORAGE = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "planning" / "storage.js"
)
PLANNING_SELECTION = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "planning" / "selection.js"
)
PLANNING_API = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "planning" / "api.js"
)
PLANNING_INTERACTIONS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "planning" / "interactions.js"
)
PLANNING_RENDER = (
...
SUBCOMPONENTS_WORKBENCH_ROUTE = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "subcomponents-workbench.js"
)
SUBCOMPONENTS_WORKBENCH_BULK_ACTIONS = (
    REPO_ROOT
    / "src"
    / "main"
    / "ui"
...
SUBCOMPONENTS_WORKBENCH_DRAWER = (
    REPO_ROOT
    / "src"
    / "main"
    / "ui"
...
SUBCOMPONENTS_WORKBENCH_SAVED_VIEWS = (
    REPO_ROOT
    / "src"
    / "main"
    / "ui"
...
SUBCOMPONENTS_WORKBENCH_INTERACTIONS = (
    REPO_ROOT
    / "src"
    / "main"
    / "ui"
...
SUBCOMPONENTS_WORKBENCH_OPTIONS = (
    REPO_ROOT
    / "src"
    / "main"
    / "ui"
...
def test_topbar_create_menu_exists_in_authenticated_shell():
    ...
    assert (
        'class="primary"'
        in html_text[
            html_text.index('id="topbar-create-toggle"') - 80 : html_text.index(
                'id="topbar-create-toggle"'
            )
            + 120
        ]
    )
...
def test_topbar_create_menu_reuses_existing_create_modals_and_keyboard_menu_pattern():
    ...
    assert (
        'topbarCreatePanel: document.getElementById("topbar-create-panel")' in dom_text
    )
    assert (
    ...
    assert (
        'createSolutionBtn: document.getElementById("create-solution")' not in dom_text
    )
...
def test_solution_modal_workflow_moves_into_shared_entities_layer():
    ...
    assert (
        "const solutionEntityController = createSolutionEntityController({" in app_text
    )
    ...
    assert (
        "return solutionEntityController.openSolutionModal(solution, tab);" in app_text
    )
...
def test_solution_and_subcomponent_forms_include_github_repo_fields():
    ...
    assert (
        "function updateSubcomponentRepoPreview(solutionId, overrideUrl) {" in app_text
    )
...
def test_planning_person_search_is_persisted_per_space():
    ...
    assert (
        'boardState.personSearch = String(stored.personSearch || "");' in storage_text
    )
    assert 'if (target.id === "wab-person-search") {' in interactions_text
    assert (
        "persistViewState();"
        in interactions_text[
            interactions_text.index(
                'if (target.id === "wab-person-search") {'
            ) : interactions_text.index('if (target.id === "wab-new-team-name") {')
        ]
    )
...
def test_planning_window_selection_is_persisted_per_space():
    ...
    assert (
        'selected_window_id: String(state.planningWindowSelectedId || ""),' in app_text
    )
...
def test_app_shell_confirm_modal_is_required_and_never_falls_back_to_browser_confirm():
    ...
    assert (
        'console.warn("Confirm modal shell missing; canceling action.");' in modal_text
    )
...
def test_subcomponents_workbench_drawer_editor_behaviors_are_route_local():
    ...
    assert (
        "async function handleSubcomponentsWorkbenchShortcut(ctx, event) {"
        in drawer_text
    )
    ...
    assert (
        "await handleSubcomponentsWorkbenchShortcut(ctx, event);" in interactions_text
    )
...
def test_subcomponents_workbench_option_population_is_route_local():
    ...
    assert (
        'function populateSubcomponentsWorkbenchOptions(ctx, { projectOptionsHtml = "" } = {}) {'
        in options_text
    )
...
def test_deliverables_bulk_actions_use_inline_feedback():
    ...
    assert (
        'function setBulkFeedback(message, tone = "info", autoClearMs = 0)' in app_text
    )
...
def test_deliverables_inline_field_updates_use_inline_feedback():
    ...
    assert (
        'setBulkFeedback("Deliverable updated.", "success", 2200);' in interactions_text
    )
    assert (
...
def test_planning_dragging_assigned_task_moves_existing_allocation_instead_of_creating_duplicate():
    ...
    assert (
        "async function moveAssignment(allocationId, assigneeType, assigneeId"
        in api_text
    )
    assert (
        "`/planning/work-allocation/allocations/${encodeURIComponent(existing.id)}`"
        in api_text
    )
...
def test_operational_views_can_hide_completed_work_across_space():
    ...
    assert (
        'const WORKSPACE_VIEW_PREFS_KEY_PREFIX = "sipm-workspace-prefs-v1";' in app_text
    )
    ...
    assert (
        "Completed items are hidden here. Use Show Completed in the top bar" in app_text
    )
