...
def test_pm_dashboard_capacity_card_uses_planning_deliverable_assignments_only():
    ...
    assert (
        'String(allocation?.work_item_type || "").trim().toLowerCase() === "subcomponent"'
        in render_text
    )
...
def test_pm_dashboard_capacity_card_defaults_to_current_month_and_exposes_month_picker():
    ...
    assert (
        '<input type="month" value="${esc(selectedCapacityMonth)}" data-pm-dashboard-action="set-capacity-month"'
        in sections_text
    )
