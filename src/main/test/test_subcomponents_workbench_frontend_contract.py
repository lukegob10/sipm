...
def test_workbench_route_renders_project_context_as_drilldown_link():
    ...
    assert (
        "function renderSubcomponentsWorkbenchProjectLink(label, projectId) {" in text
    )
    assert (
        "function renderSubcomponentsWorkbenchSolutionLink(label, solutionId) {" in text
    )
    ...
    assert (
        "renderSubcomponentsWorkbenchProjectLink(row.project_name, row.project_id)"
        in text
    )
    assert (
        "renderSubcomponentsWorkbenchSolutionLink(row.solution_name, row.solution_id)"
        in text
    )
...
def test_workbench_project_context_link_reuses_existing_project_modal():
    ...
    assert (
        "function openSubcomponentsWorkbenchProjectDrilldown(ctx, projectId) {"
        in drawer_text
    )
    assert (
        "function openSubcomponentsWorkbenchSolutionDrilldown(ctx, solutionId) {"
        in drawer_text
    )
    assert (
    ...
    assert (
        'class="sub-workbench-context-link" data-scwb-context-action="open-project"'
        in drawer_text
    )
    assert (
        'class="sub-workbench-context-link" data-scwb-context-action="open-solution"'
        in drawer_text
    )
    ...
    assert (
        "renderSubcomponentsWorkbenchDrawerProjectLink(ctx, project, subcomponent.project_id)"
        in drawer_text
    )
    assert (
        "renderSubcomponentsWorkbenchDrawerSolutionLink(ctx, solution, subcomponent.solution_id)"
        in drawer_text
    )
    assert (
...
def test_workbench_drawer_context_surfaces_effective_repo_link():
    ...
    html_text = (REPO_ROOT / "src" / "main" / "ui" / "index.html").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        "function effectiveSubcomponentRepoInfo(solutionId, overrideUrl) {" in app_text
    )
...
def test_workbench_invalid_status_and_priority_filters_are_normalized():
    ...
    assert (
        "export function normalizeSubcomponentsWorkbenchPriorityFilter(value) {" in text
    )
    assert "const parsed = Number(raw);" in text
    assert (
        'if (!Number.isInteger(parsed) || parsed < 1 || parsed > 5) return "";' in text
    )
...
def test_workbench_restore_and_saved_view_apply_reuse_filter_normalization():
    ...
    assert (
        "function applySubcomponentsWorkbenchSavedView(ctx, savedView) {"
        in saved_views_text
    )
    assert (
        "normalizeSubcomponentsWorkbenchUiState({ persist: true });" in saved_views_text
    )
    assert (
...
def test_workbench_invalid_preset_is_auto_cleared_to_all():
    ...
    assert (
        "export const VALID_SUBCOMPONENTS_WORKBENCH_PRESETS = new Set([" in filters_text
    )
...
def test_workbench_interactions_own_filter_and_selection_bindings():
    ...
    assert (
        "function updateSubcomponentsWorkbenchSolutionOptions(ctx, projectId) {"
