...
DASHBOARD_COMMON = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "dashboard" / "common.js"
)
DASHBOARD_PREFS = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "dashboard" / "prefs.js"
)
DASHBOARD_MODAL = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "dashboard" / "modal.js"
)
...
DASHBOARD_RENDER = (
    REPO_ROOT / "src" / "main" / "ui" / "js" / "routes" / "dashboard" / "render.js"
)
...
def test_dashboard_route_uses_single_customize_tables_control_with_section_tabs():
    ...
    assert (
        'class="dashboard-config-helper-link" data-dashboard-action="select-all-solutions"'
        in modal_text
    )
    assert (
        'class="dashboard-config-helper-link" data-dashboard-action="clear-solutions"'
        in modal_text
    )
    assert (
...
def test_dashboard_route_renders_solution_and_project_names_as_drilldown_links():
    ...
    assert (
        'dashboardProjectLinkMarkup(row.projectId, row.projectName, " strong")'
        in common_text
    )
    assert (
        '<div class="dashboard-cell-meta">${dashboardProjectLinkMarkup(row.projectId, row.projectName, " secondary")}</div>'
        not in render_text
    )
...
def test_dashboard_prefs_are_scoped_per_space():
    ...
    assert (
        "function ensurePrefsLoaded(dashboardState, spaceId = currentDashboardSpaceId(dashboardState)) {"
        in prefs_text
    )
...
def test_dashboard_stale_solution_selections_are_auto_cleared():
    ...
    assert (
        "const normalizedIds = filteredIds.length ? filteredIds : null;" in prefs_text
    )
