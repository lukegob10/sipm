...
PROJECTS_PACKAGE = REPO_ROOT / "src" / "main" / "backend" / "app" / "routes" / "projects"
...
LEGACY_PROJECTS_MODULE = REPO_ROOT / "src" / "main" / "backend" / "app" / "routes" / "projects.py"
...
def test_projects_routes_are_split_into_a_package_with_compatibility_reexports():
    ...
    assert 'router.add_api_route("", list_projects' in init_text
    assert 'router.add_api_route("/", list_projects' in init_text
    assert 'router.add_api_route(\n    "",\n    create_project,' in init_text
    assert 'router.add_api_route(\n    "/",\n    create_project,' in init_text
