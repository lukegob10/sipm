...
async def test_projects_import_updates_creates_and_exports(client):
    ...
    assert (
        await client.get(
            "/project-manager/api/projects", params={"sponsor": "coo office"}
        )
    ).json() == []
    ...
    updated = await client.get(
        f"/project-manager/api/projects/{existing['project_id']}"
    )
    ...
    filtered = await client.get(
        "/project-manager/api/projects", params={"status_filter": "on_hold"}
    )
    assert filtered.status_code == 200
    assert [p["project_name"] for p in filtered.json()] == ["Data Platform"]

    sponsor_filtered = await client.get(
        "/project-manager/api/projects", params={"sponsor": "cfo office"}
    )
    assert sponsor_filtered.status_code == 200
    assert [p["project_name"] for p in sponsor_filtered.json()] == ["Data Platform"]

    fallback_sponsor_filtered = await client.get(
        "/project-manager/api/projects", params={"sponsor": "test user"}
    )
    assert fallback_sponsor_filtered.status_code == 200
    assert [p["project_name"] for p in fallback_sponsor_filtered.json()] == [
        "No Sponsor"
    ]
...
async def test_update_project_rejects_name_conflict(client):
    ...
    resp = await client.patch(
        f"/project-manager/api/projects/{p2['project_id']}",
        json={"project_name": p1["project_name"]},
    )
