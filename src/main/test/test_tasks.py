...
from backend.app.models import Phase, Task
...
@pytest.mark.anyio
async def test_create_and_list_tasks(client, db_sessionmaker):
    ...
    list_resp = await client.get(f"/project-manager/api/solutions/{solution['solution_id']}/tasks")
...
@pytest.mark.anyio
async def test_create_task_defaults_assignee_and_priority(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)

    resp = await client.post(
        f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
        json={"task_name": "Minimal Task"},
    )
    assert resp.status_code == 201, resp.text
    data = resp.json()
...
@pytest.mark.anyio
async def test_task_crud_normalizes_names_and_clears_stale_blocker_note(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
    ...
    assert blank_create.json()["detail"] == "task_name is required"

    create = await client.post(
        f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
        json={
            "task_name": "  Trimmed Task  ",
            "blocked": True,
    ...
    task = create.json()
    assert task["task_name"] == "Trimmed Task"
    assert task["blocked"] is True
    assert task["blocker_note"] == "Waiting on access"
    ...
    assert blank_update.json()["detail"] == "task_name is required"
...
async def test_task_repo_override_inherits_overrides_and_clears(client, db_sessionmaker):
    ...
    update_resp = await client.patch(
        f"/project-manager/api/tasks/{created['task_id']}",
...
@pytest.mark.anyio
async def test_solution_repo_update_refreshes_cached_task_repo_inheritance(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    clear_cache()
    try:
        project, _ = await create_project_solution(client)
...
@pytest.mark.anyio
async def test_task_rejects_invalid_github_repo_override(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
...
@pytest.mark.anyio
async def test_list_all_tasks_filter_by_assignee(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
    ...
    assert (
        (await client.post(
            f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
            json={"task_name": "Task B", "assignee": "Engineer B"},
        )).status_code
        == 201
    )

    all_resp = await client.get("/project-manager/api/tasks")
    assert all_resp.status_code == 200
    assert len(all_resp.json()) == 2

    filtered = await client.get("/project-manager/api/tasks", params={"assignee": "Engineer A"})
...
@pytest.mark.anyio
async def test_list_tasks_includes_actionability_metadata(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
    today = date.today()
    ...
    with db_sessionmaker() as session:
        stale = (
            session.query(Task)
            .filter(Task.task_name == "Stale Task")
            .first()
        )
    ...
    list_resp = await client.get("/project-manager/api/tasks")
    assert list_resp.status_code == 200, list_resp.text
    rows = {item["task_name"]: item for item in list_resp.json()}
...
@pytest.mark.anyio
async def test_batch_update_tasks(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
...
@pytest.mark.anyio
async def test_batch_reopening_tasks_clears_completed_at(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
...
@pytest.mark.anyio
async def test_update_task_status_logs_completed_at_audit_field(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)
    ...
    created_resp = await client.post(
        f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
        json={
            "task_name": "Audit Completion Task",
            "status": "to_do",
            "assignee": "Engineer A",
        },
    )
...
@pytest.mark.anyio
async def test_batch_update_tasks_preserves_inherited_repo_metadata(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    project, _ = await create_project_solution(client)
...
@pytest.mark.anyio
async def test_member_can_view_task_activity(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    ...
    try:
        fastapi_app.dependency_overrides[deps_module.current_space] = lambda: SpaceContext(
            space_id="space-task-activity",
            space_name="Task Activity Space",
        ...
        create_resp = await client.post(
            f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
            json={"task_name": "Activity Task", "status": "to_do"},
        )
        assert create_resp.status_code == 201, create_resp.text
        task_id = create_resp.json()["task_id"]
        update_resp = await client.patch(
            f"/project-manager/api/tasks/{task_id}",
        ...
        activity_resp = await client.get(f"/project-manager/api/tasks/{task_id}/activity")
...
@pytest.mark.anyio
async def test_task_uniqueness_and_soft_delete(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    _, solution = await create_project_solution(client)

    payload = {"task_name": "Billing UI", "assignee": "Engineer A"}
    assert (await client.post(f"/project-manager/api/solutions/{solution['solution_id']}/tasks", json=payload)).status_code == 201
    dup = await client.post(f"/project-manager/api/solutions/{solution['solution_id']}/tasks", json=payload)
    assert dup.status_code == 400

    # soft delete
    created = (await client.get(f"/project-manager/api/solutions/{solution['solution_id']}/tasks")).json()[0]
    delete_resp = await client.delete(f"/project-manager/api/tasks/{created['task_id']}")
    assert delete_resp.status_code == 204

    get_resp = await client.get(f"/project-manager/api/tasks/{created['task_id']}")
...
@pytest.mark.anyio
async def test_soft_deleted_project_hides_task_reads_and_clears_task_cache(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    clear_cache()
    try:
        ...
        create_resp = await client.post(
            f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
            json={"task_name": "Hidden Child Task", "assignee": "Engineer A"},
        )
        assert create_resp.status_code == 201, create_resp.text
        task = create_resp.json()
        ...
        primed_list = await client.get("/project-manager/api/tasks")
        assert primed_list.status_code == 200, primed_list.text
        assert [row["task_id"] for row in primed_list.json()] == [task["task_id"]]

        primed_solution_list = await client.get(
            f"/project-manager/api/solutions/{solution['solution_id']}/tasks"
        )
        assert primed_solution_list.status_code == 200, primed_solution_list.text
        assert [row["task_id"] for row in primed_solution_list.json()] == [task["task_id"]]
        ...
        primed_export = await client.get("/project-manager/api/tasks/export")
        ...
        assert detail_resp.json()["detail"] == "Task not found"

        export_resp = await client.get("/project-manager/api/tasks/export")
...
@pytest.mark.anyio
async def test_soft_deleted_solution_hides_task_reads_and_clears_task_cache(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    clear_cache()
    try:
        ...
        create_resp = await client.post(
            f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
            json={"task_name": "Hidden After Solution Delete", "assignee": "Engineer A"},
        )
        assert create_resp.status_code == 201, create_resp.text
        task = create_resp.json()
...
@pytest.mark.anyio
async def test_member_can_delete_task(client, db_sessionmaker):
    seed_phases(db_sessionmaker)
    original_current_space = fastapi_app.dependency_overrides.get(deps_module.current_space)
    try:
        fastapi_app.dependency_overrides[deps_module.current_space] = lambda: SpaceContext(
            space_id="space-delete-task",
            space_name="Delete Task Space",
            is_global_admin=False,
            space_role="space_admin",
        )
        _, solution = await create_project_solution(client)
        create_resp = await client.post(
            f"/project-manager/api/solutions/{solution['solution_id']}/tasks",
            json={"task_name": "Delete Guard Task"},
        )
        assert create_resp.status_code == 201, create_resp.text
        task_id = create_resp.json()["task_id"]
        ...
        allowed = await client.delete(f"/project-manager/api/tasks/{task_id}")
